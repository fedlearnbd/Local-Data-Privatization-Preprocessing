...
from utils.preprocessing import direct_noise_injection
...
from utils.distances import (calculate_wasserstein,calculate_kl_divergence,calculate_ks_test)

from utils.preprocessing import preprocess

# === Argument Parser ===

parser = argparse.ArgumentParser(description="Analyse des distances entre données originales et privées (LDPP)")

parser.add_argument('--dataset', type=str, choices=['pathmnist', 'organamnist', 'bloodmnist'], default='bloodmnist', help='Dataset à utiliser')

parser.add_argument('--epsilon', type=float, default=7.0, help="Valeur de l'epsilon pour LDPP")

parser.add_argument('--noise', type=str, choices=['laplace', 'gaussian'], default='laplace', help="Type de bruit")

parser.add_argument('--samples', type=int, default=1000, help="Nombre d’échantillons à évaluer")
...
# === Application of Naive - DP ===

print(f"Application of Naive-DP with ε={args.epsilon}, Noise ={args.noise} - (Please Wait for Calculation, that's could take a moment)")

privatized_images = direct_noise_injection(train_images, args.noise, args.epsilon)
...
print(f"\n--- Averaged results For {args.samples} images ---")
